module.exports = async (req, res) => {
  ...
  minExpiry.setDate(today.getDate() + 5);
  const maxExpiry = new Date(today);
  maxExpiry.setDate(today.getDate() + 30);
...
{"i":<index>,"impact":"bearish|bullish","category":"geopolitical|macro|earnings|commodities|crypto|forex|merger|general","oneLiner":"<under 12 words describing market impact>","plays":[{"direction":"LONG|SHORT|COMMODITY","optionDirection":"call|put","ticker":"<US ticker symbol>","name":"<company or ETF name>","rationale":"<one sentence why>","conviction":<1-10>,"targetExpiry":"<YYYY-MM-DD>","type":"equity|etf|commodity|crypto|bond"}]}

Conviction scoring rules (1-10 scale) — be precise, vary your scores:
- 8-10 (HIGH): Direct earnings miss/beat, M&A announcement, FDA approval/rejection, Fed rate decision, major geopolitical escalation with clear sector impact
- 5-7 (MEDIUM): Indirect macro impact, sector rotation catalyst, analyst upgrade/downgrade, mixed signals
- 1-4 (LOW): Speculative, distant catalyst, weak correlation to ticker

optionDirection rules:
- LONG play → "call"
- SHORT play → "put"
- COMMODITY play → use "call" if bullish on commodity, "put" if bearish

targetExpiry rules:
- Must be between ${minStr} and ${maxStr}
- Pick the nearest Friday (weekly expiry) that gives at least 5 days
- For high conviction plays use the nearest weekly
- For medium/low conviction give more time, use 2-3 weeks out

Rules:
- 1-2 plays per story max
- Use DIFFERENT conviction levels across stories
- Use real US-listed tickers only (options-eligible equities or ETFs)
